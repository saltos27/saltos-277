...
const start = (kill = new Client()) => {
    console.log(color('\n> DEV OFICIAL ='), color('saltos', 'yellow'))
	console.log(color('\n>'), color('Instalacion finalizada, ya puede usar comandos...\n', 'red'))
        ...
        kill.onAddedToGroup(((chat) => {
            ...
            if (totalMem < 20) { 
            	kill.sendText(chat.id, `Un nuevo grupo, uwu! 😃\nLástima que no tenga el requisito, que es tener al menos [100] miembros. Tienes ${totalMem}, reune más gente! 😉`).then(() => kill.leaveGroup(chat.id))
            } else {
